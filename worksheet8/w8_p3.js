window.onload = function init (){
  ...
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, texSize, texSize, 0, gl.RGB, gl.UNSIGNED_BYTE, red);
  ...
  visibilityLoc = gl.getUniformLocation(program, "visibility"); // get location of visibility in shader
  ...
  gl.uniform1f(visibilityLoc, 1.0); // visibility


  gl.uniform1i(gl.getUniformLocation(program, "texMap"), 0);
  gl.drawArrays(gl.TRIANGLE_FAN, 0, 4);

  v = mult(v, translate(vec3(light_pos[0], light_pos[1] - offset, light_pos[2])));
  v = mult(v, mshadow);
  v = mult(v, translate(vec3(-light_pos[0], -light_pos[1] - offset, -light_pos[2])));
  ...
  gl.uniform1f(visibilityLoc, 0.0); // not visibility
